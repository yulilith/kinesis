...
export default function CommunityNav({ active }: { active: "agents" | "threads" | "network" }) {
  ...
  return (
    <div className="w-full mb-8 flex items-center gap-4 border-b border-border">
      {TABS.map((t) => {
        ...
        return (
          <Link
            key={t.href}
            href={t.href}
            className={`py-2 text-sm font-light tracking-wide -mb-px border-b-2 transition-colors ${
              isActive
